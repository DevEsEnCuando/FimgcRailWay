...
import { Tab, Tabs, Container, Form, ListGroup,OverlayTrigger, InputGroup, Button, Row, Col, Table, Modal, Alert, Spinner } from 'react-bootstrap';
...
const GenerarMemorandum = () => {
    ...
    const verificarExp = async () => {
        ...
        try {
            const response = await axios.get(`http://localhost:8080/api/expediente/${inputNroDoc}/V`);
    ...
    const handleGenerar = () => {
        ...
        if (tramiteExpediente === 'Plan de Tesis' || tramiteExpediente === 'Obtención de Título Profesional' || tramiteExpediente == "Obtención de Bachiller") {
            ...
            axios
                .post(`http://localhost:8080/api/expediente/${nroDoc}/actualizarMiembros?miembroId=${docentesSeleccionados[0].id}&presidenteId=${docentesSeleccionados[1].id}`)
                .then((response) => {
    ...
    const enviarCorreo = async () => {
        ...
        try {
            ...
            if (uploadResponse.data && tramiteExpediente === 'Obtención de Bachiller') {
                ...
                const destinatarios = docentesSeleccionados.map((docente) => docente.email);
                            ...
                            <fieldset className='customize-fielset'>
                                    ...
                                    <Col sm="10">
                                        <InputGroup>
                                            <OverlayTrigger placement="top" delay={{ show: 170, hide: 430 }} overlay={(props) => renderTooltip(props, 'expediente')}>
                                                <Form.Control type="text" aria-label="Ingrese el número de expediente."
                                                    id="numeroExp"
                                                />
                                            </OverlayTrigger>
                                ...
                                {tramiteExpediente == "Plan de Tesis" && (
                                    <>
                                        {
                                            <Form.Group as={Row} className="mb-3">
                                                <Form.Label column sm="2" className="fw-bold customize-label">Miembros</Form.Label>
                                                <Col sm="10">
                                                    <InputGroup>
                                                        ...
                                                        <tbody>
                                                            {showAlertaExp && alertaExp.type === 'success' && (
                                                                <tr>
                                                                    <td>{asesor}</td>
                                                                    <td>
                                                                        <Form.Select size='sm' aria-label="Seleccione rol" defaultValue="Secretario de Facultad" disabled>
                                                                            <option value="Secretario de Escuela">Presidente</option>
                                                                            <option value="Secretario de Facultad">Miembro</option>
                                                                        </Form.Select>
                                                                    </td>
                                                                    <td></td>
                                                                </tr>
                                                            ...
                                                            {docentesSeleccionados.map((docente, index) => (
                                                                <tr key={index}>
                                                                    ...
                                                                    <td>
                                                                        <Form.Select size='sm' aria-label="Seleccione rol">
                                ...
                                <div className="col-md-2 mx-auto">
                                    <Button variant="success btn-into-row-2" onClick={handleGenerar}>Generar <i className="fi fi-bs-document"></i></Button>
                                </div>
