...
import { Tab, Tabs, Container, Form, ListGroup, InputGroup, Button, Row, Col, Table, Modal, Alert } from 'react-bootstrap';
  ...
  const verificarExp = async () => {
    ...
    try {
      ...
      if (veriResponse.data.tramite.nombre === 'Plan de Tesis' || veriResponse.data.tramite.nombre === 'Borrador de Tesis') {
        ...
        destinatarios.push(veriResponse.data.presidente, veriResponse.data.interesado, veriResponse.data.miembro, veriResponse.data.asesor);
        ...
        // Mover esta línea de código debajo de setTramiteExpediente
        destinatarios.push(veriResponse.data.presidente, veriResponse.data.interesado, veriResponse.data.miembro);
        console.log(destinatarios);
  ...
  const enviarCorreo = async () => {
    ...
    try {
      ...
      if (uploadResponse.data && tramiteExpediente === 'Obtención de Bachiller') {
        ...
        const correos = destinatarios.map((destinatario) => destinatario.email);
      ...
      <Container>
        <Form>
          ...
          <fieldset className='customize-fielset'>
            <legend className='customize-legend-2 text-uppercase'>Generar resolución | All Tramites</legend>
        ...
        <Modal aria-labelledby="modal-de-destinatarios" size="lg" centered show={modalDestinatarios}>
          ...
          <Modal.Body>
            ...
            <div className='customize-btn-div justify-content-end'>
              <Button className='customize-btn-1' onClick={enviarCorreo}>Enviar </Button>
            </div>

            {/* Agrega este botón para enviar el correo */}
          </Modal.Body>
        </Modal>
      </Container>
